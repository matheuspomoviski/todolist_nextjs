...
const tasksController = {
    ...
    getTasks: async (req, res) => {
        try {
            ...
            if (tasks.length === 0) {
                // Corrigido para verificar se o array está vazio
            ...
            res.status(500).json({
                mensagem:
                    "Houve um erro ao tentar consultar as tarefas desse usuário",
            });
        }
    },
    update: async (req, res) => {
        try {
            ...
            try {
                ...
                console.log(error);
                res.status(500).json({
                    mensagem: "Houve um erro ao tentar atualizar a tarefa",
                });
            }
        } catch (error) {
            console.log(error);
            res.status(500).json({
                mensagem: "Houve um erro ao receber os dados da tarefa",
            });
        }
    },
    ...
    get: async (req, res) => {
        try {
            ...
            res.status(500).json({
                mensagem: "Houve um erro ao tentar buscar a tarefa",
            });
        }
    },
    delete: async (req, res) => {
        try {
            ...
            res.status(500).json({
                mensagem: "Houve um erro ao tentar deletar a tarefa",
            });
        }
    },
};
